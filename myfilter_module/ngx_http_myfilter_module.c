...
static ngx_int_t ngx_http_myfilter_init(ngx_conf_t* cf)
{
    //insert head filter to the head of the list
    ngx_http_next_header_filter = ngx_http_top_header_filter;
    ngx_http_top_header_filter = ngx_http_myfilter_header_filter;

    //insert body filter to the head of the list
...
static ngx_int_t ngx_http_myfilter_header_filter(ngx_http_request_t *r)
{
    ...
    //if failed, call next header filter
    ...
    && ngx_strncasecmp(r->headers_out.content_type.data, (u_char*)"text/plain", sizeof("text_plain")-1) == 0)
    {
        ctx->add_prefix = 1;
